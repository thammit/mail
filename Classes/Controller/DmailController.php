...
class DmailController extends AbstractController
{
            ...
            case 'info':
                ...
                if ($this->createMailFrom_UID && !$quickmail['send']) {
                    ...
                    if (is_numeric($newUid)) {
                        ...
                        // fetch the data
                        if ($this->fetchAtOnce) {
                            $fetchMessage = MailUtility::fetchUrlContentsForDirectMailRecord($row, $this->params);
    ...
    protected function compileQuickMail(array $row, string $message): array|string
    {
        ...
        } else if (!str_contains(base64_decode($htmlmail->theParts['plain']['content']), '<!--DMAILER_SECTION_BOUNDARY')) {
    ...
    public function makeCategoriesForm(array $row, array $indata): array|string
    {
        ...
        if (is_array($indata['categories'])) {
            ...
            $theOutput = MailUtility::fetchUrlContentsForDirectMailRecord($row, $this->params);
        }

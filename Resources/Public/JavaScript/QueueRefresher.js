define([], function () {
  ...
  document.querySelectorAll(selector).forEach((runningProgressBar) => {
    const interval = setInterval(() => {
      window
        .fetch(
          TYPO3.settings.ajaxUrls.mail_queue_state +
            "&mail=" +
            parseInt(runningProgressBar.dataset.mailUid),
        )
        .then((response) => {
          if (!response.ok) {
        ...
        .then((result) => {
          ...
          const numberOfRecipients = parseInt(
            runningProgressBar.getAttribute("aria-valuemax"),
          );
          runningProgressBar.style.width = `${deliveryProgress}%`;
          runningProgressBar.innerText = `${deliveryProgress}%`;
          runningProgressBar.setAttribute(
            "title",
            `${recipientsHandled}/${numberOfRecipients}`,
          );
          runningProgressBar.setAttribute(
            "aria-valuenow",
            `${deliveryProgress}%`,
          ...
          const scheduledBegin = tableRow.querySelector(
            ".mail-scheduled-begin",
          );
          if (scheduledBegin) {
          ...
          if (status === 5) {
            ...
            // }
            // const mailReportDeleteButton = tableRow.querySelector(
            //   ".mail-report-delete-button",
            // );
